...
BASE_OUTPUT_DIR="Test/GC0.4-0.6_Run3/ED4"
...
for JOB_ID in 1 2 3; do
    ...
    OUT_DIR="$BASE_OUTPUT_DIR/N${N}_job${JOB_ID}"
    mkdir -p "$OUT_DIR"
    
    # Absolutize OUT_DIR because IndexGen runs from the root or wherever it is called
    ABS_OUT_DIR="$(realpath "$OUT_DIR")"
    CONFIG_PATH="$ABS_OUT_DIR/config_N${N}_job${JOB_ID}.json"
    
    echo "Preparing job ${JOB_ID} for N=$N with $RUNS iterations in $ABS_OUT_DIR"
...
"
    ...
    if [ $? -ne 0 ]; then
        echo "Failed to generate config for N=$N job ${JOB_ID}"
        continue
    fi

    # Create distinct SLURM submission script
    SLURM_SCRIPT="$ABS_OUT_DIR/submit_N${N}_job${JOB_ID}.sh"

    cat << EOF > "$SLURM_SCRIPT"
#!/bin/bash
#SBATCH --job-name=IG_N${N}_job${JOB_ID}
...
echo "Target N=$N, Job $JOB_ID, Total Runs=$RUNS"
