...
GraphLD::GraphLD() : V(), graph(V, vector<pair<int, long double>>())
{
}

GraphLD::GraphLD(int V) : V(V), graph(V, vector<pair<int, long double>>())
{
}
...
GraphInt::GraphInt() : V(), graph(V, vector<pair<int, int>>())
{
}

GraphInt::GraphInt(int V) : V(V), graph(V, vector<pair<int, int>>())
{
}
					 ...
					 GraphLD &graph)
{
	...
	for (vector<CondProb>::const_reverse_iterator rit = prob.rbegin(); rit != prob.rend(); rit++)
	{
		...
		for (auto &strMap : (*rit))
		{
			...
			{ // single letter string
				...
				for (auto &toPair : currentMap)
				{
					...
					if (toPair.first.empty())
					{ // to string is the empty string. connect to carries
						for (auto &pr : carryVs)
						{
					...
					{ // to string is not empty. connect to respective vertex
			...
			{ // string size > 1. connect only the last letter to all non empty strings in map.
			  // if empty string in map, connect last letter to carries. add first letter vertex to currentLevel
				...
				vertexIndex--;
				for (int strIndex = currentStrsize - 2; strIndex >= 0; strIndex--)
				{ // connect each letter, except last, to the one after with weight 0
		...
		{ // level has empty string
			...
			if (findEmptyString == emptyStrMapIt->second.end())
			{ // empty string doesn't have empty string in its map
				carryVs.clear();
			}
			else
			{ // empty string has empty string in its map.
		...
		{ // no empty string so nothing to carry
			carryVs.clear();
		}
				  ...
				  const int letterNumTarget)
{
	...
	if (V == (int)minLetters1.size() - 1)
	{ // if vertex is end vertex it's ok
		return true;
	}
									  ...
									  const vector<int> &maxLetters1, const int stringLen)
{
	...
	for (int i = 0; i < K; i++)
	{
		...
		for (auto &mp : dp[i])
		{
			...
			for (auto &edge : adj1[fromV])
			{
				...
				if (not IsGoodVertex(toV, minLetters1, maxLetters1, i + 1, stringLen))
				{ // letters so far: i plus 1 for toV
					continue;
				}
	...
	for (int j = K; j > 1; j--)
	{ // don't need first and last END_SYMBOL
