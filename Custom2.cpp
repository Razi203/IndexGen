...
bool checkWord(const std::vector<int> &word, int n, int a)
{
    ...
    if (((weighted_sum % modulus) + modulus) % modulus != ((a % modulus) + modulus) % modulus)
    {
        return false;
    }

    // Check if the sum is equal to the reminder modulo n+1
    long long sum = 0;
    for (int val : word)
    {
        sum += val;
    }
    if ((sum % modulus + modulus) % modulus != ((a % modulus) + modulus) % modulus)
    {
        return false;
    }
 ...
 */
void findCodeWordsWorker(int n, int a, unsigned long long start_index, unsigned long long count,
...
std::vector<std::string> GenerateCustomCodes2(int n, int a, unsigned int num_threads)
{
    ...
    for (unsigned int i = 0; i < threads_to_use; ++i)
    {
        ...
        threads.emplace_back(findCodeWordsWorker, n, a, start_index, count, &thread_results[i]);
        start_index += count;
    }
