...
struct PeqIndex
{
    ...
    PeqIndex() = default;
    explicit PeqIndex(const std::string &pattern)
    {
        build(pattern);
    }
...
struct PatternHandle
{
    ...
    explicit PatternHandle(const std::string &p)
    {
        build(p);
    }
...
inline int MyersSingleWord(const PatternHandle &H, const std::string &text)
{
    ...
    for (unsigned char tc : text)
    {
        ...
        score += int((HP >> H.highestShift) & 1U) - int((HN >> H.highestShift) & 1U);
    }
    return score;
}
...
inline int MyersMultiWord(const PatternHandle &H, const std::string &text)
{
    ...
    for (unsigned char c : text)
    {
        ...
        // branchless top-bit update
        score += int((lastHP >> H.highestShift) & 1U) - int((lastHN >> H.highestShift) & 1U);
    }
    return score;
}

// -------------------- Compose Eq for Hyyrö/GKR band from PeqIndex --------------------
inline uint64_t ComposeEqBand(const PeqIndex &P, unsigned char tch, int /*m*/, int j, int c)
{
    ...
    {
        uint64_t topMask = base[bTop] & ((off == 63) ? ~0ULL : ((1ULL << (off + 1)) - 1ULL));
        Eq |= (topMask << (63 - off));
    }
    if (off < 63)
    {
...
// Gropl, Klau, Reinert and Hyrro Algorithm for banded edit distance using bit-parllelism + Ukkonen's band
inline int GKRHyyroSingleWordBand(const std::string &text, const PatternHandle &H, int k)
{
    using Bit = uint64_t;
...
// Thresholded/banded API (primary), reuses handle. Use for repeated computations with same pattern.
inline int EditDistanceBanded(const std::string &text, const PatternHandle &H, int k)
{
    return GKRHyyroSingleWordBand(text, H, k);
}

// One-shot helpers (build handle on the fly)
inline int EditDistanceBanded(const std::string &a, const std::string &b, int k)
{
...
// Exact distance using same handle (no banding). Use for repeated computations with same pattern.
